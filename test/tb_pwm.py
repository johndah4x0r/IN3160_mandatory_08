...
from cocotb.triggers import ClockCycles, First, FallingEdge, RisingEdge
from cocotb.triggers import ReadOnly, Timer, with_timeout, SimTimeoutError
...
class MessageQueue(Queue):
    ...
    def find_error(self, dut, error_type):
        ...
        raise AssertionError(
            "{err} error sought, but not found!".format(err=error_type)
        )
...
class Monitor:
    ...
    async def check_short_circuit(self):
        ...
        while True:
            await self.dut.dir.value_change
            try:
                if self.dut.reset.value == 0:
                    ...
                    assert (
                        self.en_mon.stable_interval("ns") > PERIOD_NS - 1
                    ), "SHORT CIRCUIT DANGER: en deactivated less than one cycle before dir change"
                    wait_task = Timer(PERIOD_NS - 1, "ns")
                    event_task = self.dut.en.value_change
                    result = await First(wait_task, event_task)
                    assert (
                        result == wait_task
                    ), "SHORT CICUIT DANGER: En was not stable for {per} {uni}".format(
                        per=PERIOD_NS, uni="ns"
                    )
            except AssertionError as e:
    ...
    async def check_timeout(self):
        ...
        while True:
            if self.dut.duty_cycle.value == 0:
                await self.dut.duty_cycle.value_change
            try:
                await with_timeout(self.dut.en.value_change, PWM_TIMEOUT_MS, "ms")
            except SimTimeoutError:
                self.messages.put_message(
                    TIMEOUT_TYPE, "PWM signal is static, TB timed out "
                )
    ...
    async def check_direction(self):
        ...
        while True:
            ...
            await ClockCycles(
                self.dut.mclk, 2
            )  # Trigger two clock edges after duty cycle was changed
            await ReadOnly()  # Wait for all signals to settle (all delta delays)
            try:
                duty = int(self.dut.duty_cycle.value.to_signed())  # Numpy compatibility
                if np.int8(duty) > 0:
                    assert (
                        self.dut.dir.value == 1
                    ), "DIR is not '1' within 2 clock cycles of positive duty cycle: {DU} = {D}".format(
                        DU=np.int8(duty), D=self.dut.duty_cycle.value
                    )
                if np.int8(duty) < 0:
                    assert (
                        self.dut.dir.value == 0
                    ), "DIR is not '0' within 2 clock cycles of negative duty cycle: {DU} = {D}".format(
                        DU=np.int8(duty), D=self.dut.duty_cycle.value
                    )
            except AssertionError as e:
        ...
        while True:
            ...
            try:
                ...
                if self.duty_mon.stable_interval("us") > interval:
                    assert (
                        interval > TOO_FAST_PWM_US
                    ), "PWM period too short!: {iv:.2f}us, f={f:.3f}kHz   Minimum period: {per} us, ({maxf:.2f}kHz) ".format(
                        iv=interval,
                        f=(1000 / interval),
                        per=TOO_FAST_PWM_US,
                        maxf=(1000 / TOO_FAST_PWM_US),
                    )
                    ...
                    # newer versions of Python/numpy, may need to change the line below.
                    # set_duty = np.int8(np.int16(self.dut.duty_cycle.value.signed_integer)*100/128)
                    set_duty = (
                        np.int8(self.dut.duty_cycle.value.signed_integer) * 100 / 128
                    )

                    # Report duty cycle and check correspondens betweem input and output
                    sign = "-" if self.dut.dir.value == 0 else " "
                    self.dut._log.info(
                        "Duty cycles: Set dc: {S:.1f}%, Measured dc: {Sig}{M:.1f}%, period = {P:.1f}us, f = {F:.2f}kHz".format(
                            S=set_duty,
                            Sig=sign,
                            M=measured_duty,
                            P=interval,
                    ...
                    assert (
                        deviation < 5
                    ), "Set and measured duty cycle deviates by more than 5% ({D}%) ".format(
                        D=deviation
                    )
            except AssertionError as e:
...
class FaultInjector:
    ...
    async def short_2(self):
        ...
        self.dut._log.info(
            "Injecting error: pulse deassertion < 1 cycle before direction change"
        )
        await self.disable_reset()
        self.dut.en.value = Force(1)
        await Timer(1, "ns")
    ...
    async def short_3(self):
        ...
        self.dut._log.info(
            "Injecting error: pulse deassertion < 1 cycle after direction change"
        )
        await self.disable_reset()
        self.dut.en.value = Force(0)
        await Timer(20, "ns")
    ...
    async def duty(self):
        ...
        self.dut._log.info(
            "Injecting error: Setting one duty cycle and pulsing another"
        )
        await self.disable_reset()
        self.dut.en.value = Force(0)
